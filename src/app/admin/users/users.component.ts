...
import { ViewDeletedUserComponent } from './view-deleted-user/view-deleted-user.component';
...
export class UsersComponent extends AppComponentBase implements AfterViewInit {
    ...
    @ViewChild('viewDeletedUser', { static: true }) viewDeletedUser: ViewDeletedUserComponent;
@ViewChild("specialPriviledgeModal", { static: true }) specialPriviledgeModal: ModalDirective;
    uploadUrl: string;
    ...
    listofSpecialPriviledges = [];
    user = this.appSession.user.userName;
    ...
    viewSpecialPriviledge(username): void{

this._userServiceProxy.getSpecialPriviledgeByUser(username).subscribe( c => {
    this.listofSpecialPriviledges = c;

});


        this.specialPriviledgeModal.show();
    }
    ...
    deleteUser(user: UserListDto): void {
        if (user.userName === AppConsts.userManagement.defaultAdminUserName) {
            this.message.warn(this.l('{0}UserCannotBeDisable', AppConsts.userManagement.defaultAdminUserName));
            return;
        }

        this.message.confirm(
            this.l('UserDisableWarningMessage', user.userName),
            this.l('AreYouSure'),
            (isConfirmed) => {
                if (isConfirmed) {
    ...
    closePriviledge(){
        this.specialPriviledgeModal.hide();
    }
    onShown(){
